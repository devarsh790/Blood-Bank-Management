...
def send_donor_email(recipient_email, donor_id, dname):
    try:
        ...
        msg = EmailMessage()
        msg.set_content(f"Hello {dname},\n\nThank you for registering to donate blood!\nYour assigned Donor ID is: {donor_id}\n\nPlease keep this ID handy for your donation appointment.\n\nBest Regards,\nBlood Bank Management System")
        msg['Subject'] = "Blood Bank - Your Donor ID"
...
def donate():
    if request.method  == 'POST':
        ...
        donor_id = cur.lastrowid # Get the newly assigned ID
        
        conn.commit()
        conn.close()
        
        # Send Email
        send_donor_email(demail, donor_id, dname)
        
        flash(f'Success! Donor details Added. Your Donor ID is {donor_id}. An email has been sent to {demail}.','success')
